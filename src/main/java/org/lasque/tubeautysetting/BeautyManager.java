...
public class BeautyManager implements Beauty {
    ...
    private TusdkImageFilter.MapPropertyBuilder mFilterMapProperty;
    ...
    private void changedFilter(final String code) {
        mRenderPipe.getRenderPool().runSync(new Runnable() {
            @Override
            public void run() {
                ...
                if (!TextUtils.isEmpty(code)) {
                    ...
                    mFilterMapProperty = new TusdkImageFilter.MapPropertyBuilder(filter.getProperty(TusdkImageFilter.PROP_PARAM));
                }
            }
        });
    }
    ...
    public void updateFilterFromMap(){
        mRenderPipe.getRenderPool().runAsync(new Runnable() {
            @Override
            public void run() {
                int index = mFilterMap.get(SelesParameters.FilterModel.Filter);
                Filter filter = mFP.getFilter(index);
                if (filter != null){
                    filter.setProperty(TusdkImageFilter.PROP_PARAM,mFilterMapProperty.makeProperty());
                }
            }
        });
    }

    private void changedSkinMode(final BeautySkinMode mode) {
    ...
    private void updateSticker(final long stickerId) {
        mRenderPipe.getRenderPool().runSync(new Runnable() {
            @Override
            public void run() {
    ...
    private void updateMonsterFace(final String code) {
        mRenderPipe.getRenderPool().runSync(new Runnable() {
            @Override
            public void run() {
                int monsterIndex = mFilterMap.get(SelesParameters.FilterModel.MonsterFace);
    ...
    @Override
    public Map<String, Double> getCurrentFilterDefaultKeyValue() {
        return mFilterMapProperty.pars;
    }
    ...
    @Override
    public void setFilterValue(String key, double strength) {
        mFilterMapProperty.pars.remove(key);
        mFilterMapProperty.pars.put(key,strength);
        updateFilterFromMap();
    }
    ...
    @Override
    public void setJoiner(final RectF videoRect, final RectF cameraRect, final String videoPath) {
        mRenderPipe.getRenderPool().runSync(new Runnable() {
            @Override
            public void run() {
    ...
    @Override
    public boolean joinerSeek(final long ts) {
    ...
    @Override
    public void updateJoiner(final RectF videoRect, final RectF cameraRect) {
    ...
    @Override
    public void setJoinerPlayerState(final boolean isPlaying) {
        if (mFP.getFilter(DOUBLE_VIEW_INDEX) == null) return;
        mRenderPipe.getRenderPool().runSync(new Runnable() {
            @Override
            public void run() {
