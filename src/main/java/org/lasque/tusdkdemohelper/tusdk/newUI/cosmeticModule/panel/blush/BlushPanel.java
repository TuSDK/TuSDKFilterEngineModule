    ...
    protected View createView() {
        ...
        mAdapter.setOnItemClickListener(new OnItemClickListener<BlushAdapter.BlushViewHolder,CosmeticTypes.BlushType>() {
            ...
            public void onItemClick(int pos, BlushAdapter.BlushViewHolder holder, CosmeticTypes.BlushType item) {
                mCurrentType = item;
                mController.getEngine().controller().changeCosmetic(TuFilterCombo.TuCosmeticMode.Blush,StickerLocalPackage.shared().getStickerGroup(item.mGroupId).stickers.get(0).categoryId,-1,1, TuFilterCombo.TuCosmeticLipGlossStyle.None);
                //todo 设置美妆
